...
function strip(item){
  ...
  return {
    ...
    assigneeContactId:node.assigneeContactId,
    contractorContactId:node.contractorContactId,
    completionState:node.completionState,
    workflowStatus:node.workflowStatus,
    completionSubmittedAt:node.completionSubmittedAt || null,
    actualFinishDay:node.actualFinishDay || null,
    ...
    workTasks:(node.workTasks || []).filter(task => !task.trashed).map(task => ({
      ...
      completionSubmittedAt:task.completionSubmittedAt || null,
      actualFinishDay:task.actualFinishDay || null,
      ...
      executionReports:task.executionReports || [],
      executionComments:task.executionComments || [],
