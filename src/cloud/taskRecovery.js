...
export function taskRecordFreshness(task){
  ...
  let latest=Math.max(timestamp(task.updatedAt),timestamp(task.createdAt),timestamp(task.completedAt),timestamp(task.at));
  const scan=array=>{
...
export function mergeRecoveredTasks(...groups){
  ...
  groups.flat().forEach(task=>{
    const value=task&&normalize(task),id=String(value?.id||'');
    if(!id)return;
    const current=byId.get(id);
    if(!current||taskRecordFreshness(value)>taskRecordFreshness(current))byId.set(id,value);
  });
  return [...byId.values()];
}
