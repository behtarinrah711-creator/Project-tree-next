...
export function installNotebookWorkspace({documentRef=globalThis.document,windowRef=globalThis.window,repository=createNotebookRepository()}={}){
  ...
  function applySurface(){const on=onRoute();documentRef.body?.classList.toggle('global-surface',on);page.classList.toggle('hidden',!on||/\/export/i.test(windowRef.location.hash||''));exportPage?.classList.toggle('hidden',!/\/notebook\/export/i.test(windowRef.location.hash||''));if(on&&!/\/export/i.test(windowRef.location.hash||''))render();if(/\/notebook\/export/i.test(windowRef.location.hash||''))renderExport();windowRef.KarhaWorkspaceChrome?.updateWorkspaceContextBar?.();}
  const openNotebook=()=>windowRef.location.hash='#/notebook',openExport=()=>windowRef.location.hash='#/notebook/export';
  windowRef.addEventListener('karha:open-notebook',()=>{windowRef.KarhaWorkspaceChrome?.enterGlobalSurface?.();const route='#/notebook';windowRef.KarhaBrowserHistory?.push?.(windowRef.KarhaBrowserHistory.stateForRoute?.({projectId:null,moduleId:'notebook',hash:route})||{hash:route},route)||(windowRef.location.hash=route);applySurface();});
