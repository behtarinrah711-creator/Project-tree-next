...
export function installNotebookWorkspace({documentRef=globalThis.document,windowRef=globalThis.window,repository=createNotebookRepository()}={}){
  ...
  function bind(body){
    ...
    body.querySelector('[data-clear-completed]')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();confirmAction('همه موارد انجام‌شده حذف شوند؟',()=>{repository.mutate(nb=>{if(starredMode){for(const notebookList of nb.lists||[])trashCompletedItems(notebookList.items,{starredOnly:true});}else trashCompletedItems(active(nb)?.items);});render();});});
