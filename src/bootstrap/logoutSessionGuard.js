...
const isDevicePersistent = key => DEVICE_PERSISTENT_KEYS.has(key) || key.startsWith(`${STORAGE_KEYS.notebook}:user:`);
...
export function clearAppSessionCache(storage){
  ...
  for(let i=0;i<storage.length;i++){
    ...
    if(key && !isDevicePersistent(key) && APP_STORAGE_PREFIXES.some(prefix=>key.startsWith(prefix))) keys.push(key);
  }
