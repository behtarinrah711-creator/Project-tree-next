...
import { findInTree, canHoldWorkTasks, isStage } from '../domain/wbs/normalize.js';
...
function updateWork(nodes, workId, updater){
  ...
  const visit = (nodes || []).map(node => {
    if(String(node?.id) === String(workId) && (canHoldWorkTasks(node) || isStage(node))){
      changed = true;
      return updater(node);
    }
...
export class WorkTaskRepository{
  ...
  work(projectId, workId){
    ...
    return found && (canHoldWorkTasks(found.item) || isStage(found.item)) ? found.item : null;
  }
